...
resource "aws_cloudformation_stack_set_instance" "default" {
  for_each = var.enable_stackset && var.create_instance ? toset(var.stackset_instance_account_id) : []
  ...
  stack_set_name = var.enable_stackset && length(aws_cloudformation_stack_set.default) > 0 ? aws_cloudformation_stack_set.default[0].name : null
}